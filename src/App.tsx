...
import { api } from "./api";
import { TranscriptSegment, ChatMessage, AppView, AIMode, AIModel, Toast } from "./types";
import {
...
export default function App() {
  ...
  // Refs so event-listener callbacks always see current values
  ...
  const handleAskAI = useCallback(async (q: string) => {
    ...
    // Update ref synchronously so re-entrant calls from auto-ask are blocked
    ...
    <div className="app" style={{ opacity: opacity / 100 }}>
      ...
      <div className="titlebar" onPointerDown={handleDragStart} data-tauri-drag-region>
        ...
        <button className="titlebar-btn"
          ...
          {view === "settings" ? <IconArrowLeft size={13} /> : <IconSettings size={13} />}
        </button>
      ...
      {view === "main" && (
        <>
          ...
          <div className="controls">
            {!isRecording ? (
              <button className="btn btn-record" onClick={handleStartRecording} title="Start capturing system audio (WASAPI loopback)">
                <IconMic size={12} /> <span>Record</span>
              </button>
            ) : (
              <button className="btn btn-stop" onClick={handleStopRecording} title="Stop recording">
            ...
            <button className={`btn ${autoAsk ? "btn-auto-on" : "btn-ghost"}`}
              ...
              title={autoAsk ? "Auto-ask on — AI responds after every sentence" : "Enable auto-ask"}
            ...
            <button className={`btn ${useScreen ? "btn-screen-on" : "btn-ghost"}`}
              ...
              title={useScreen ? "Screen context on" : "Include screenshot with AI requests"}
            ...
            {segments.length > 0 && (
              <button className="btn btn-ghost" onClick={handleGenerateNotes}
                disabled={aiThinking} title="Generate structured meeting notes">
            ...
            <button className="btn btn-ghost" onClick={handleClearSession} title="Clear transcript and chat">
              <IconTrash size={12} />
            </button>
          </div>

          {/* Content: transcript + chat */}
          <div className="main-body">

            {/* Live transcript */}
            <div className="transcript-pane" role="log" aria-live="polite" aria-label="Live transcript">
            ...
            <div className="chat-pane">
                  ...
                  <div key={m.id} className={`chat-msg chat-msg-${m.role}`}>
                    {m.role === "user" ? (
                      ...
                      <>
                        ...
                        <button className="copy-msg-btn" title="Copy response"
              ...
              <div className="chat-input-row">
                ...
                <button className="send-btn" onClick={handleSend}
                  disabled={aiThinking || !chatInput.trim()} title="Send (Enter)">
        ...
        <div className="settings-view">
            ...
            <div className="model-picker">
              {(["gpt-4o-mini", "gpt-4o"] as AIModel[]).map(m => (
                <button key={m} className={`model-btn${aiModel === m ? " active" : ""}`}
                  onClick={() => setAiModel(m)} aria-pressed={aiModel === m}>
                  {m}
                </button>
              ))}
            </div>
          ...
          <div className="settings-section">
            ...
            <p className="settings-hint" style={{ lineHeight: 1.8 }}>
              ...
              screen-capture API. Audio is captured at OS level via WASAPI loopback and streamed
              to Deepgram over an encrypted WebSocket.
            </p>
          </div>
        </div>
      )}
